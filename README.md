...
# How does this work
There is a launcher `aoc.py` written in Python that assumes Python is used for
the solution.

## Day folders
Each day is a Python module under `src/days` called `dayXX` where `XX` is the
...
command line as a positional parameter.

# Solving a puzzle

Let's assume Python is the language of choice. The day starts with a module
that has stubs for the two solvers and 4 tests:
...
Return the solution as a string, the launcher will prerry print it for easy
copying into the puzzle form.

# Solved Puzzles

When a puzzle is fully solved, the day module can have a file called `SOLVED`
to signal this easily.
